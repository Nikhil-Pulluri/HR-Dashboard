...
export default function Bookmarks() {
  ...
  return (
    <div className="overflow-auto p-6 flex flex-col space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">{users.length > 0 ? users.map((user) => <UserCard key={user.id} user={user} />) : <SkeletonCard />}</div>
    </div>
  )
}
